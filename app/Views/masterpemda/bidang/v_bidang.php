                ...
                <table style="font-size: 12px;" class="table-striped table-sm table table-hover" id="table-1">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <form action="<?= htmlentities(base_url('master/subbidangp/'. $db->id)); ?>" method="post">
                                    <?= csrf_field() ;?>
                            ...
                            <td class="text-right">
                                <button type="button" onclick="f_editbidangp(<?= htmlentities($db->id) ;?>)" class="btn btn-success btn-sm" data-bs-toggle="modal" data-bs-target="#staticBackdropEdit" data-toggle="tooltip" title="Edit bidang : <?= htmlentities($db->nama_bidang); ?>">
                                    <i class="fas fa-pencil-alt"></i>
                                </button>
                            </td>
                            <td>
                                <a href="<?= htmlentities(base_url('master/hapusbidangp/'. $db->id)); ?>" style="cursor: pointer;" class="btn btn-sm btn-danger btn-hapus" data-toggle="tooltip" title="Hapus bidang : <?= htmlentities($db->nama_bidang); ?>">
...
<!-- Modal Edit -->
    ...
    // $(document).ready(function () {
        ...
        function f_editbidangp($id){
            $.ajax({
                type: "get",
                url: "<?= htmlentities(base_url("master/f_editbidangp")) ?>/" + $id,
                // data: "data",
                // dataType: "dataType",
        ...
        $('#tombolEdit').on('click', function () {
            ...
            $.ajax({
                type: "GET",
                url: "<?= htmlentities(base_url('master/proccesseditbidangp')); ?>",
                data: {
                    id: $id,
                    kd: $kd,
        ...
        $('#tombolSimpan').on('click', function() {
            ...
            $.ajax({
                type: "GET",
                url: "<?= htmlentities(base_url('master/proccesstambahbidangp')); ?>",
                data: {
                    kd: $kd,
